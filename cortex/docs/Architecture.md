...
- Новые узлы: SpecSynth (синтез оракулов) и TestSynth (генерация приёмочных тестов) вставлены перед Dev для повышения надёжности автопрогонов. QA запускает все тесты из каталога артефактов.
