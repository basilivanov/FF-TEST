...
### Added
- **Cortex Health Monitoring System**: Автоматическая система анализа полноты и качества документации кортекса
  - `CortexAnalyzer` службa для анализа файлов, ролей и системного покрытия
  - Ежечасный cron job для автоматического мониторинга состояния кортекса
  - REST API endpoints (`/cortex/health`, `/cortex/analyze`, `/cortex/history`, `/cortex/roles/{role}`)
  - Интеграция с UI: замена моковых данных реальными метриками на странице документации
  - База данных SQLite для хранения истории анализов и метрик по ролям
  - Скрипт `scripts/cortex-analyzer.sh` для автономного запуска анализа
  - Тестовый набор `test_cortex_analyzer.py` для проверки всех компонентов

### Changed
- **Docs UI**: Трансформирована страница документации в dashboard мониторинга здоровья кортекса
- **API Router**: Добавлен `cortex_health_router` в основное FastAPI приложение
- **Cron Jobs**: Настроен автоматический анализ кортекса каждый час (`0 * * * *`)

### Technical Details
- Анализируется качество содержимого файлов на основе структуры, актуальности и специфичности
- Метрики включают: overall score, context freshness, role coverage, content quality по ролям
- Поддерживаются роли: architect, dev, qa, scribe, maintainer
- Автоматическая генерация рекомендаций по улучшению документации
- Еженедельная очистка старых отчётов (>30 дней)
