...
# Поиск файлов с исключениями
echo "Поиск файлов с исключениями..."
FILES=$(find "$ROOT_DIR" $EXCLUDE_PARAMS $EXT_PARAMS -type f | sort)
...
echo "$FILES" | while read file; do
    if [ -n "$file" ]; then
        ...
        if [ $SIZE -le $CURRENT_MAX_BYTES ] && [ $LINES -le $CURRENT_MAX_LINES ]; then
            PREVIEW=$(head -n $CURRENT_MAX_LINES "$file" | jq -sR .)
        else
...
echo "$FILES" | while read file; do
    if [ -n "$file" ]; then
        ...
        if [ $SIZE -le $CURRENT_MAX_BYTES ] && [ $LINES -le $CURRENT_MAX_LINES ]; then
            echo '```' >> "$OUTPUT_MD"
            head -n $CURRENT_MAX_LINES "$file" >> "$OUTPUT_MD"
            echo '```' >> "$OUTPUT_MD"
        else
