...
# Чтение параметров политики из YAML файла с помощью yq или альтернативного метода
if command -v yq &> /dev/null; then
    # Если yq доступен, используем его
    EXCLUDE_DIRS=($(yq eval '.exclude_dirs[]' "$CONFIG_FILE"))
    INCLUDE_EXT=($(yq eval '.include_ext[]' "$CONFIG_FILE"))
    MAX_LINES=$(yq eval '.max_lines' "$CONFIG_FILE")
    MAX_BYTES=$(yq eval '.max_bytes' "$CONFIG_FILE")
    OPENAPI_MAX_LINES=$(yq eval '.openapi_caps.max_lines' "$CONFIG_FILE")
    OPENAPI_MAX_BYTES=$(yq eval '.openapi_caps.max_bytes' "$CONFIG_FILE")
    FOLLOW_SYMLINKS=$(yq eval '.follow_symlinks' "$CONFIG_FILE")
else
    # Альтернативный метод парсинга YAML
    EXCLUDE_DIRS=("$(grep -A 15 "exclude_dirs:" "$CONFIG_FILE" | grep '^- ' | sed 's/^- //' | tr '\n' ' ')")
    INCLUDE_EXT=("$(grep -A 20 "include_ext:" "$CONFIG_FILE" | grep '^- ' | sed 's/^- //' | tr '\n' ' ')")
    MAX_LINES=$(grep "max_lines:" "$CONFIG_FILE" | awk '{print $2}')
    MAX_BYTES=$(grep "max_bytes:" "$CONFIG_FILE" | awk '{print $2}')
    OPENAPI_MAX_LINES=$(grep -A 2 "openapi_caps:" "$CONFIG_FILE" | grep "max_lines:" | awk '{print $2}' | sed 's/,//')
    OPENAPI_MAX_BYTES=$(grep -A 2 "openapi_caps:" "$CONFIG_FILE" | grep "max_bytes:" | awk '{print $2}')
    FOLLOW_SYMLINKS=$(grep "follow_symlinks:" "$CONFIG_FILE" | awk '{print $2}')
    
    # Преобразование строк в массивы
    read -a EXCLUDE_DIRS <<< "$EXCLUDE_DIRS"
    read -a INCLUDE_EXT <<< "$INCLUDE_EXT"
fi
...
if [ -n "$EXCLUDE_PARAMS" ] && [ -n "$EXT_PARAMS" ]; then
    # Удаляем последний -o из EXCLUDE_PARAMS
    EXCLUDE_PARAMS="${EXCLUDE_PARAMS%-o}"
    FILES=$(find "$ROOT_DIR" -type d \( $EXCLUDE_PARAMS \) -prune -o -type f \( $EXT_PARAMS \) -print | sort)
elif [ -n "$EXT_PARAMS" ]; then
    FILES=$(find "$ROOT_DIR" -type f \( $EXT_PARAMS \) -print | sort)
else
    FILES=$(find "$ROOT_DIR" -type f -print | sort)
fi
...
# Подсчет файлов для JSON отчета
FILES_TOTAL=$(echo "$FILES" | grep -v "^$" | wc -l)

# Сбор информации о файлах и создание JSON отчета
SANITY_LIST="["
i=0
echo "$FILES" | while IFS= read -r file; do
    if [ -n "$file" ] && [ $i -lt 30 ]; then
...
# Создание JSON отчета с правильными данными
cat > "$OUTPUT_JSON" << EOF
{
  "files_total": $FILES_TOTAL,
  "excluded_dirs": ["$(IFS='","'; echo "${EXCLUDE_DIRS[*]}")"],
